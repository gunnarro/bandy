...
public class TrainingsTable {
	...
	static {
		...
		DATABASE_CREATE_QUERY.append(",").append("UNIQUE (").append(COLUMN_FK_TEAM_ID).append(",").append(COLUMN_START_DATE).append(") ON CONFLICT REPLACE);");
	}

	public static void onCreate(SQLiteDatabase database) {
		CustomLog.i(TrainingsTable.class, DATABASE_CREATE_QUERY.toString());
		database.execSQL(DATABASE_CREATE_QUERY.toString());
	}

	public static void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion) {
		CustomLog.i(TrainingsTable.class, "Upgrading database from version " + oldVersion + " to " + newVersion + ", which will destroy all old data");
