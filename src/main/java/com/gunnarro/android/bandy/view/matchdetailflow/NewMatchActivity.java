...
public class NewMatchActivity extends FragmentActivity implements NoticeDialogListener {
	...
	private void setInputValue(int id, Item item) {
		...
		if (inputView != null && item != null) {
			inputView.setText(item.getValue().trim());
		} else {
			CustomLog.e(this.getClass(), "No input field found or value is equal to null, for id: " + id + ", value: " + item);
		}
	}
	...
	private void setupEventHandlers() {
		...
		// Match type
		Item[] matchTypes = bandyService.getMatchTypes();
		ImageButton typeBtn = (ImageButton) findViewById(R.id.matchSelectTypeBtnId);
		typeBtn.setOnClickListener(new SelectDialogOnClickListener(getSupportFragmentManager(), matchTypes, R.id.matchTypeTxt, false));

		// Home Team
		ImageButton homeBtn = (ImageButton) findViewById(R.id.matchSelectHomeTeamBtnId);
		homeBtn.setOnClickListener(new SelectDialogOnClickListener(getSupportFragmentManager(), bandyService, SelectionListType.TEAM_NAMES, null,
				R.id.matchHomeTeamTxt, false));
		// Away team
		ImageButton awayBtn = (ImageButton) findViewById(R.id.matchSelectAwayTeamBtnId);
		awayBtn.setOnClickListener(new SelectDialogOnClickListener(getSupportFragmentManager(), bandyService, SelectionListType.TEAM_NAMES, null,
				R.id.matchAwayTeamTxt, false));
		// Venue
		Item[] venueNames = new Item[] { new Item(1, "Bergbanen"), new Item(2, "Voldsløkka") };
		...
		refereeBtn.setOnClickListener(new SelectDialogOnClickListener(getSupportFragmentManager(), bandyService, SelectionListType.REFEREE_NAMES, null,
	...
	private void save() {
		...
		if (getInputValue(R.id.matchRefereeNameTxt) != null) {
			// referee = new Referee(getInputValue(R.id.matchRefereeNameTxt),
			// getInputValue(R.id.matchRefereeNameTxt));
		}
