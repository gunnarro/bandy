...
public class BandyRepositoryImpl implements BandyRepository {
	...
	public Item[] getClubAsItems() {
		...
		if (cursor != null && cursor.getCount() > 0) {
			cursor.moveToFirst();
			clubs = new Item[cursor.getCount()];
			int i = 0;
			while (!cursor.isAfterLast()) {
				String fullClubName = cursor.getString(cursor.getColumnIndex(ClubsTable.COLUMN_CLUB_NAME)) + " "
						+ cursor.getString(cursor.getColumnIndex(ClubsTable.COLUMN_CLUB_DEPARTMENT_NAME));
				clubs[i] = new Item(cursor.getInt(cursor.getColumnIndex(TableHelper.COLUMN_ID)), fullClubName, false);
				i++;
				cursor.moveToNext();
			}
		}
	...
	public Item[] getSportTypes() {
		return getTypes(SportTypesTable.TABLE_NAME);
	}
	...
	private Item[] getTypes(String tableName) {
		Item[] types = new Item[] {};
		...
		if (cursor != null && cursor.getCount() > 0) {
			types = new Item[cursor.getCount()];
			int i = 0;
			cursor.moveToFirst();
			while (!cursor.isAfterLast()) {
				int id = cursor.getInt(cursor.getColumnIndex(TableHelper.COLUMN_ID));
				String value = cursor.getString(cursor.getColumnIndex(TypesHelperTable.COLUMN_TYPE));
				types[i] = new Item(id, value);
				cursor.moveToNext();
				i++;
			}
		}
	...
	public Item[] getMatchStatusList() {
		...
		if (cursor != null && cursor.getCount() > 0) {
			matchStatuses = new Item[cursor.getCount()];
			int i = 0;
			cursor.moveToFirst();
			while (!cursor.isAfterLast()) {
				int id = cursor.getInt(cursor.getColumnIndex(TableHelper.COLUMN_ID));
				String value = cursor.getString(cursor.getColumnIndex(TypesHelperTable.COLUMN_TYPE));
				matchStatuses[i] = new Item(id, value);
				cursor.moveToNext();
				i++;
			}
		}
	...
	public Item[] getTeamAsItems(Integer clubId) {
		...
		query.append(" AND c.").append(TableHelper.COLUMN_ID).append(" LIKE '").append(id).append("'");
		query.append(" ORDER BY t.").append(TeamsTable.COLUMN_TEAM_NAME).append(" ASC");
		CustomLog.e(this.getClass(), "sqlQuery=" + query.toString());
		this.database = getDatabase(false);
		Cursor cursor = this.database.rawQuery(query.toString(), null);
		if (cursor != null && cursor.getCount() > 0) {
			teams = new Item[cursor.getCount()];
			cursor.moveToFirst();
			int i = 0;
			while (!cursor.isAfterLast()) {
				teams[i] = new Item(cursor.getInt(cursor.getColumnIndex(TableHelper.COLUMN_ID)), cursor.getString(cursor
						.getColumnIndex(TeamsTable.COLUMN_TEAM_NAME)), false);
				i++;
				cursor.moveToNext();
			}
		}
		CustomLog.e(this.getClass(), "clubId=" + clubId + ", teams=" + cursor.getCount());
		if (cursor != null && !cursor.isClosed()) {
			cursor.close();
		}
		return teams;
	...
	public String[] getTeamNames(Integer clubId) {
		...
		query.append(" ORDER BY t.").append(TeamsTable.COLUMN_TEAM_NAME).append(" ASC");
		return getTeamNames(query.toString(), new String[] { clubId != null ? clubId.toString() : "%" });
	}
